...
class Movie:
    ...
    def _read_movie_header(self):
        ...
        elif pixel_mode_code == 4:
            self.pixel_mode = 'MONO12_to_MONO8'
        else:
            raise Exception('Invalid pixel mode: Must be 1 for MONO8, 2 for MONO16BE, 3 for MONO16LE, and 4 for MONO12 (temporary solution)')
...
#            self.length_data = read('I')
    ...
    def read_uint12(data_chunk):
        r"""
        Read MONO12 packed bytes into uint12 (embedded into uint16) values.
        """
        data = np.array(data_chunk, dtype=np.uint8)
    ...
    @staticmethod
    def convert_uint12_to_uint8(data_chunk):
        r"""
        Temporary method, uses that first and third byte in a triplet have
        very similar distributions (encoding unknown) to reconstruct an uint8
        image of the uint12 (packed) input, by discarding the middle byte.
        """
        data = np.array(data_chunk, dtype=np.uint8)
        fst_uint8, mid_uint8, lst_uint8 = np.reshape(data, (data.shape[0] // 3, 3)).T
        return np.reshape(np.concatenate((fst_uint8[:, None], lst_uint8[:, None]), axis=1), 2 * fst_uint8.shape[0])
    ...
    def get_frame(self, i):
        ...
        if self.pixel_mode == 'MONO16LE':
            return np.reshape( struct.unpack('<' + (self.length_data//2)*"H", self.file.read(self.length_data)), (self.size_y, self.size_x) ).astype('H')
        if self.pixel_mode == 'MONO12_to_MONO8':
            #TODO Temporary solution, reduced precision, converting to MONO8
            tmp = self.convert_uint12_to_uint8(struct.unpack('>' + self.length_data*"B", self.file.read(self.length_data))).reshape((self.size_y, self.size_x))
            return tmp
